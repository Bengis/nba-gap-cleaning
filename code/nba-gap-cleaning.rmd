...
```{r, echo=TRUE}
# read data
nba_org <- read.csv("../data/nba-stats_out.csv")
nba_org["sex"] <- NA
nba_org$sex<-"0"
wnba_org <- read.csv("../data/wnba-stats_out.csv")
wnba_org["sex"] <- NA
wnba_org$sex<-"1"
...
Este dataset es interesante tanto para aficionados a la NBA y a la WNBA como para personas que quieran obtener datos acerca de la brecha salarial en los deportes profesionales.
...
El código en R esta incluido en este fichero con extensión rcmd y tambien se puede descargar en GitHub desde la siguiente dirección:
...
```{r ,eval=TRUE,echo=TRUE}
write.csv2(t_nba, file = "../data/nba_out.csv")
```
